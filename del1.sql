...
CREATE TABLE IF NOT EXISTS alimama_kgb_algo_dev.kgb_match_qy_opt03_train_samples_hash_columnized_weighted_neg
(
...
) PARTITIONED BY (ds STRING) LIFECYCLE 12; 
...
INSERT OVERWRITE TABLE alimama_kgb_algo_dev.kgb_match_qy_opt03_train_samples_hash_columnized_weighted_neg PARTITION(ds='${bizdate}')
SELECT sample_id
...
where ds = '${bizdate}_weighted_root_leafcate' and tag = 'train'
    and ((CAST(unseen_flag as BIGINT) != -1 and CAST(unseen_flag as BIGINT) != -2) or
         (CAST(unseen_flag as BIGINT) == -1 and cast(GET_JSON_OBJECT(sample_id, '$.score_rank') as bigint) <= 1) or
         (CAST(unseen_flag as BIGINT) == -2 and ((cast(GET_JSON_OBJECT(sample_id, '$.neg_type') as bigint) == 0 and cast(GET_JSON_OBJECT(sample_id, '$.neg_rk') as bigint) <= 1) 
                                                or (cast(GET_JSON_OBJECT(sample_id, '$.neg_type') as bigint) == 1 and cast(GET_JSON_OBJECT(sample_id, '$.neg_rk') as bigint) <= 1)))
        ) -- 0-1训练时 注意控制样本量
