...
class DatasetManager(object):
    ...
    def get_popularity(self):
        ...
        self.popularity = self.npy_loader.load(const_util.popularity) #ml有4819个item，self.popularity.shape=4819
...
class DICESampler(SAMPLER.Sampler):
    ...
    def generate_negative_samples(self, user, pos_item):
        ...
        num_pop_items = len(pop_items) #比anchor更流行margin的样本数量
        ...
        num_unpop_items = len(unpop_items) #比anchor更不流行margin的样本数量

        if num_pop_items < self.pool: #流行数量小于池子，都从不流行池子里采样
        ...
        elif num_unpop_items < self.pool: #流行数量大于池子，不流行数量小于池子，都从流行里采样
